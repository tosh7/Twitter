...
extension ApiClient {
    ...
    func getMe(_ request: MeRequest, completion: @escaping ((Result<User, Error>) -> Void)) { makeGetRequest(request: request, completion: completion) }
}
...
extension ApiClient {
    ...
    func getMe(_ request: MeRequest) async -> Result<User, Error> { await self.makeGetRequest(request: request) }
}
